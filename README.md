...
4. ImageFlux Live StreamingのAPIトークンを取得します。
5. オブジェクトストレージのサイトアカウントを作成し、認証情報を取得します。
6. コンテナレジストリとそのユーザを作成し、webappおよびserverのコンテナイメージをbuild、pushします。
7. WireGuardクライアントを準備し、VPNルータに設定する公開鍵を取得します。
8. Terraformで残りの基盤を構築します。
9. さくらのウェブアクセラレータのCNAMEおよびAppRun専有型のIPアドレスに、ドメインを割り振ります。
10. さくらのウェブアクセラレータのLet's Encryptを有効化、サイト自体も有効化します。
11. WireGuardを用いてVPN接続し、データベースにテーブルを作成します。
12. AppRun専有型のアプリケーションをアクティブにします。
